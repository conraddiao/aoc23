const data = `Game 1: 8 green; 5 green, 6 blue, 1 red; 2 green, 1 blue, 4 red; 10 green, 1 red, 2 blue; 2 blue, 3 red
...
function advent(string) {
    let games = string.replaceAll(/\s/g, '').trim().split(/Game\d*:/);
    games.shift();
    const res = games.map((game) => {
        return game.split(';').reduce((minTriple, hand) => {
            const red = hand.match(/\d*(?=red)/) == null ? 0 : Number(hand.match(/\d*(?=red)/)[0]);
            const green = hand.match(/\d*(?=green)/) == null ? 0 : Number(hand.match(/\d*(?=green)/)[0]);
            const blue = hand.match(/\d*(?=blue)/) == null ? 0 : Number(hand.match(/\d*(?=blue)/)[0]);
            const handTriple = { red: red, green: green, blue: blue };

            return minTriple = [
                Math.max(minTriple[0], handTriple.red),
                Math.max(minTriple[1], handTriple.green),
                Math.max(minTriple[2], handTriple.blue)
            ];
        }, [0, 0, 0]);
    })

    console.log(res);
    console.log(res.map((game) => game.reduce((acc, curr) => acc * curr, 1)).reduce((acc, curr) => acc + curr, 0));
}
