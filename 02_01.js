...
function advent(string, redTest, greenTest, blueTest) {
    let games = string.replaceAll(/\s/g, '').trim().split(/Game\d*:/);
    games.shift();
    const resultArray = games.map((game) => {
        return game.split(';').reduce((handTruth, hand) => {
                ...
                const handResult = red <= redTest && green <= greenTest && blue <= blueTest;
                console.log(red, green, blue, handResult);
                return handTruth && handResult;
            }, true);
        });
        console.log('\n'+resultArray+'\n');
        let sum = 0;
