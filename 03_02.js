...
const data = `...788.............................54.........501...........555.........270.................................521......893....................
...
function advent2(string) {
    ...
    //find each possible gear
    const asterixIndicies = [...string.matchAll(`\\*`)].map((iter) => iter.index);

    //find each real gear
    const gears = asterixIndicies.map((c, i, a) => {
        const n =  c - width;
        const e =  c + 1;
        const s =  c + width;
        const w =  c - 1;
        ...
        const sw = c - width - 1;
        const nw = c + width - 1;
        const dirIndices = [n, e, s, w, ne, se, sw, nw]
        const dirValues = dirIndices.map((i) => chars[i]).map((v) => v === undefined ? '.' : v)
        //console.log([chars[c], dirValues]);
        
        const touches = dirIndices
            .filter((i) => /\d/.test(chars[i]))
            .sort(function(a, b) {
                return a - b;
        }).map((c, i, a) => [c, i == 0 || c != (a[i - 1] + 1)]).filter((i) => i[1]).map((touch) => touch[0])
        ...
        return {
            ...
            dirValues: dirValues,
            touches: touches,
            isGear: touches.length > 1
        }
    }).filter((res) => res.isGear == true)
    
    console.log(gears[0])

    //for each real gear: for each touch: evauate the number the touch is a part of, multiply the numbers, reduce the results by addition

    return gears.map((gear) => {        
        const pow = gear.touches.map((i) => {
            let num =[chars[i]];
            let tmp = [];
            ...
            num = Number([...tmp.reverse(), ...num].join(''))
            return num;
        }
        ).reduce((num, pow) => num = num * pow, 1)
    return pow;
